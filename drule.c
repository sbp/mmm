#define VERSION "0.2 12-Jun-2010"
/*****************************************************************************/
...
#define MAX_THEOREMS 10000
  ...
  while (1) {
    ...
    if (strlen(edit(str1, 8)) == 0) continue; /* Ignore blank lines */

    /* Get theorem */
    let(&theorem, str1);
    while (instr(1, str1, ";") == 0) {
      if (linput(NULL, NULL, &str1) == NULL) {
        printf("?Error in: %s\n", theorem);
...
#define GENERATE 0
   ...
   if (!GENERATE) {
    ...
    i = strlen(proofBody);
    let(&proofComment, cat("; ! ", str(i), i == 1 ? " step" : " steps",
        NULL));
...
#define MAX_LINE_LENGTH 69
    printf("\n");
    while (strlen(theoremBody) > 0) {
      for (i = MAX_LINE_LENGTH; i >= 0; i--) {
        if (i > strlen(theoremBody)) {
          i = strlen(theoremBody);
          break;
        }
        if (i == 0) {
          p = strlen(theoremBody);
          break;
        }
        if (theoremBody[i - 1] == ' ') {
          p = i;
      ...
      if (strlen(theoremBody) + strlen(theoremComment) < MAX_LINE_LENGTH) {
    ...
    if (strlen(theoremComment) > 0) printf("%s\n", theoremComment);
    ...
    if (i == i) {   /* Change to != if "real" theorem not wanted */
      for (i = 1; i <= strlen(printedProvedTheorem); i++) {
        ...
        if (i < strlen(printedProvedTheorem)
    ...
    while (strlen(proofBody) > 0) {
      for (i = MAX_LINE_LENGTH; i >= 0; i--) {
        if (i > strlen(proofBody)) {
          p = strlen(proofBody);
          break;
        }
        p = i;
      ...
      if (strlen(proofBody) + strlen(proofComment) < MAX_LINE_LENGTH) {
    ...
    if (strlen(proofComment) > 0) printf("%s\n", proofComment);
...
/*d*//*for shortest 123 study*/
    /*if (strlen(proof1) > 21) printf("try ");*/
    /*printf("fin %ld %s %s\n", strlen(proof1), proof1, step[0]);*/
...
/*d*//*for shortest 123 study*/
        ...
        /*if (strlen(str1) > 21) printf("try ");*/
        /*print2("maj %ld %s %s\n", strlen(str1), str1, step[majorPremiseStep]);*/
        /* Skip the subproof for the major premise to find the
           minor premise */
        ...
        /*if (strlen(str1) > 21) printf("try ");*/
        /*print2("min %ld %s %s\n", strlen(str1), str1, step[minorPremiseStep]);*/
        ...
        let(&antecedent, mid(str1, 2,
            strlen(str1) - strlen(substitutedStep[i]) - 1));
 ...
 /*D*/zzz:
    ...
    for (i = proofLen - 1; i >= 0; i--) {
      ...
      totalSize += strlen(step[i]);
      if (longestSize < strlen(step[i]))
        longestSize = strlen(step[i]);
      totalSizeSubst += strlen(substitutedStep[i]);
      if (longestSizeSubst < strlen(substitutedStep[i]))
        longestSizeSubst = strlen(substitutedStep[i]);
...
vstring dAlgorithm(vstring majorPremise, vstring minorPremise) {
  ...
  for (pos = 0; pos < clen; pos++) {
    ...
    if (c[pos] != '>' && c[pos] != '~') {
      m = toupper(c[pos]);
      if (m == c[pos]) bug(6); /* Running out of lcase letters can cause this */
      c[pos] = m;
    }
  }
...
int qsortStringCmp(const void *p1, const void *p2)
{
  ...
  i1 = strlen(*(char * const *)p1);
  i2 = strlen(*(char * const *)p2);
  if (i1 < i2) return -1;
  if (i1 > i2) return 1;
  /* Use line */
...
#define EVAL_DIR    "LRLLLLLLRLR"
  ...
  if (i >= 0) {
    ...
    let(&quantifiers, left(equation1, i + 2));
    let(&equation1, right(equation1, i + 3));
    tokens = strlen(equation1); /* Correct the length */
  }

  /* Allocate and initialize a string array for subequations */
  subExprArray = malloc(tokens * sizeof(vstring));
  ...
  if (strlen(CONNECTIVES) != strlen(CONN_ARITY)
      || strlen(CONNECTIVES) != strlen(CONN_STRENGTH)
      || strlen(STRENGTHS) != strlen(EVAL_DIR)) bug(401);
  /* Internal error check; also build list of unary connectives */
  let(&unaryConnectives, "");
  for (i = 0; i < strlen(CONNECTIVES); i++) {
    if (CONN_ARITY[i] == '1') {
        ...
        if (!strcmp(subExprArray[j], ")")) {
          ...
          if (level == 0) {
            ...
            while (subExprArray[i][0] == '('
                && subExprArray[i][strlen(subExprArray[i]) - 1] == ')' ) {
              /* Make sure we don't remove too many */
              m = 0;
              for (k = 1; k < strlen(subExprArray[i]) - 1; k++) {
            ...
            /* If we're left with one character (must be a variable), leave it
               ...
               operator */
            if (strlen(subExprArray[i]) > 1 && (strchr(unaryConnectives,
  ...
  for (strength = '9'; strength >= '/'; strength--) {
    ...
    for (subexpr = startToken; evalDir * subexpr <= evalDir * endToken;
      ...
      if (strlen(subExprArray[subexpr]) != 1) continue;
      ...
      for (i = -1; i <=1; i += 2) {
        if (arity == 2 || i == -evalDir) {
         if (strlen(subExprArray[subexpr + i]) == 1) {
  ...
  if (expr2[0] == '(') {
    ...
    let(&expr2, seg(expr2, 2, strlen(expr2) - 1));
  }
